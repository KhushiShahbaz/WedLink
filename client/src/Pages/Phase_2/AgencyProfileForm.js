...
export const AgencyProfile = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            let result;
            if (edit && id) {
                result = await dispatch(updateAgency({ id, formData: formDataToSend })).unwrap();
            } else {
                result = await dispatch(createAgency(formDataToSend)).unwrap();
            }

            if (result?.success) {
                toast.success(edit ? 'Agency updated successfully!' : 'Agency created successfully!');
                navigate('/user/profile');
            }
        } catch (err) {
            setError(err.message || 'Something went wrong');
            toast.error(err.message || 'Failed to save agency profile');
        } finally {
            setLoading(false);
        }
    };
    ...
    const validate = () => {
        ...
        } else if (!/^\d{5}-\d{7}-\d{1}$/.test(formData.cnicNo)) {
        ...
        if (!formData.yearOfExp || formData.yearOfExp < 0) {
                            ...
                            >
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="flex justify-between text-sm text-gray-500">
                                        <span>{formData.profile?.length || 0}/500 characters</span>
                                        {errors.profile && (
